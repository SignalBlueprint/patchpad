  ...
  return (
    <div className="w-80 bg-white border-l border-neutral-200 flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200">
        <h3 className="font-semibold text-neutral-900">Conversation Insights</h3>
        <button
          onClick={onClose}
      ...
      {/* Stats Summary */}
      {insights && (
        <div className="px-4 py-3 border-b border-gray-100 bg-neutral-50">
          <div className="grid grid-cols-2 gap-3">
            <div className="text-center">
              <div className="text-xl font-bold text-secondary-600">{insights.totalConversations}</div>
              <div className="text-xs text-neutral-500">Conversations</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-secondary-600">{insights.questionCount}</div>
              <div className="text-xs text-neutral-500">Questions Asked</div>
            </div>
          </div>
        </div>
      )}
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`flex-1 py-2 px-2 text-xs font-medium flex items-center justify-center gap-1 transition-colors ${
              activeTab === tab.id
                ? 'text-secondary-600 border-b-2 border-indigo-600'
        ...
        {loading ? (
          ...
          <>
            ...
            {activeTab === 'questions' && (
              <div className="p-4 space-y-3">
                {insights.topQuestions.length === 0 ? (
                  <p className="text-sm text-neutral-500 text-center py-4">No questions found</p>
                ) : (
            ...
            {activeTab === 'topics' && (
              <div className="p-4 space-y-2">
                {insights.topTopics.length === 0 ? (
                  <p className="text-sm text-neutral-500 text-center py-4">No topics found</p>
                ) : (
              ...
              <div className="p-4 space-y-3">
                {insights.knowledgeGaps.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-sm font-medium text-neutral-700">No knowledge gaps detected</p>
            ...
            {activeTab === 'activity' && (
              <div className="p-4">
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-neutral-700 mb-2">Last 30 Days</h4>
                  {/* Sparkline chart */}
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-medium text-neutral-700">Recent Activity</h4>
                  {activity.slice(-7).reverse().map((day, i) => (
                    ...
                    >
                      ...
                      <span className={`font-medium ${day.count > 0 ? 'text-secondary-600' : 'text-neutral-400'}`}>
...
function QuestionCard({
  ...
  return (
    <div className="bg-neutral-50 rounded-lg p-3 border border-gray-100">
      <div className="flex items-start gap-2">
        <div className="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-secondary-600 text-xs font-bold flex items-center justify-center">
          {rank}
        </div>
              ...
              <button
                ...
                className={`text-xs px-2 py-0.5 rounded ${
                  isGenerating
                    ...
                    : 'bg-purple-100 text-secondary-600 hover:bg-purple-200'
                }`}
            ...
            <div className="mt-2 space-y-1">
              {question.conversations.map((convId, i) => (
                <button
                  ...
                  className="block w-full text-left text-xs px-2 py-1 bg-white rounded border border-neutral-200 text-neutral-600 hover:border-indigo-300 hover:text-secondary-600 transition-colors truncate"
                >
                  Conversation {i + 1}
                </button>
              ))}
...
function TopicCard({
  ...
  return (
    <div className="flex items-center gap-3 py-1.5 group">
      <div className="flex-1 min-w-0">
        <span className="text-sm font-medium text-neutral-700 capitalize">{topic.topic}</span>
      </div>
      <div className="w-24 flex items-center gap-2">
        ...
        <span className="text-xs text-neutral-500 w-6 text-right">{topic.count}</span>
      </div>
      {onCreateBrief && (
        <button
          ...
          className={`opacity-0 group-hover:opacity-100 transition-opacity px-2 py-0.5 text-xs rounded ${
            isGenerating
              ? 'bg-neutral-100 text-neutral-400 cursor-not-allowed'
              : 'bg-indigo-100 text-secondary-600 hover:bg-indigo-200'
          }`}
